...
describe("NewHorizenProofVerifier contract", function () {
  ...
  beforeEach(async function () {
    ...
    NewHorizenProofVerifier = await ethers.getContractFactory("NewHorizenProofVerifier")
  ...
  it("should initialize correct parameters", async function () {
    expect(await verifierInstance.hasRole(ownerRole, owner.getAddress())).to.equal(true)
  })

  it("only owner can set operator", async function () {
    await verifierInstance.connect(owner).revokeRole(operatorRole, operator.getAddress())
    await verifierInstance.connect(owner).grantRole(operatorRole, addr1.getAddress())
    expect(verifierInstance.connect(addr1).grantRole(operatorRole, addr1.getAddress())).to.be.revertedWith(
      "AccessControl: account "+ (await addr1.getAddress()).toLowerCase() +" is missing role "+ ownerRole
    )
  })
  ...
  it("operator can invoke submitAttestation", async function () {
    await verifierInstance.connect(operator).submitAttestation(initialAttestationId, substrateTree.root);
    await expect(await verifierInstance.connect(operator).proofsAttestations([initialAttestationId])).to.equal(substrateTree.root)
  })

  it("non-operator cannot invoke submitAttestation", async function () {
    expect(verifierInstance.connect(owner).submitAttestation(initialAttestationId, proofsAttestations)).to.be.revertedWith(
      "AccessControl: account "+ (await owner.getAddress()).toLowerCase() +" is missing role 0x523a704056dcd17bcf83bed8b68c59416dac1119be77755efe3bde0a64e46e0c"
    );
  })

  it("operator can enable sequel attestations", async function () {
    await verifierInstance.connect(operator).flipIsEnforcingSequentialAttestations();
    await expect(await verifierInstance.connect(operator).isEnforcingSequentialAttestations()).to.equal(true)
  })

  it("non-operator cannot enable sequel attestations", async function () {
    expect(verifierInstance.connect(addr2).flipIsEnforcingSequentialAttestations()).to.be.revertedWith(
      "AccessControl: account "+ (await addr2.getAddress()).toLowerCase() +" is missing role 0x523a704056dcd17bcf83bed8b68c59416dac1119be77755efe3bde0a64e46e0c"
    );
  })
  ...
  it("when sequencing is enabled, sequence enforced on submitAttestation", async function () {
    ...
    /* Negative Case */
    await expect(verifierInstance.connect(operator).submitAttestation(initialAttestationId, substrateTree.root))
  ...
  it("non-operator cannot invoke submitAttestationBatch", async function () {
    expect(verifierInstance.connect(addr2).submitAttestationBatch([initialAttestationId], [substrateTree.root])).to.be.revertedWith(
      "AccessControl: account "+ (await owner.getAddress()).toLowerCase() +" is missing role 0x523a704056dcd17bcf83bed8b68c59416dac1119be77755efe3bde0a64e46e0c"
    );
  })

  it("submitAttestationBatch must have an equal number of ids to proofs", async function () {
    expect(verifierInstance.connect(addr2).submitAttestationBatch([initialAttestationId, initialAttestationId + 1], [substrateTree.root]))
  ...
  it("when sequencing is enabled, sequence enforced on submitAttestationBatch", async function () {
    ...
    await expect(await verifierInstance.connect(operator).proofsAttestations([3])).to.equal(substrateTree.root)

    /* Negative Case */
    ids = [5, 6, 7]
    await expect(verifierInstance.connect(operator).submitAttestationBatch(ids, roots))
      .to.be.revertedWith("InvalidAttestation")
  })
  ...
  it("verifyProofAttestation returns true for each leaf in the tree", async function () {
    ...
    for(let i = 0, j = 1; i < substrateTree.leaves.length; i++, j++) {
      let returnVal = await verifierInstance.connect(operator).verifyProofAttestation(
  ...
  it("verifyProofAttestation returns false if leafIndex is out of bounds", async function () {
    ...
    await expect(verifierInstance.connect(operator).verifyProofAttestation(
      initialAttestationId,
  ...
  it("verifyProofAttestation returns true if only one leaf and leaf matches root", async function () {
    await verifierInstance.connect(operator).submitAttestation(initialAttestationId, minSubstrateTree.root);
    await expect(await verifierInstance.connect(operator).proofsAttestations([initialAttestationId])).to.equal(minSubstrateTree.root)
